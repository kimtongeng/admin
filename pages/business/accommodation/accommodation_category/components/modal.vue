          ...
          <b-col xl="12" lg="12">
            ...
            >
              ...
              >
                ...
                <b-form-select-option :value="enums.business_type.ACCOMMODATION"
                >{{ $t('accommodation') }}
                </b-form-select-option>
              </b-form-select>
            </b-form-group>
    ...
    modalType: {
      handler(val) {
        if(val == 1) {
          ...
          this.imagePath = this.apiUrl + 'images/accommodation/category/'
          this.modal = true
          this.setData()
        }
      },
  ...
  computed: {
    modalHeader() {
      return this.modalType == 1
        ? `${this.$t('add')} ${this.$t('accommodation_category')}`
        : `${this.$t('update')} ${this.$t('accommodation_category')}`
    },
    modalOk() {
  ...
  methods: {
   ...
   save(evt) {
      ...
      vm.$validator.validateAll().then((result) => {
        if (result) {
          let url = 'accommodation_category/store'
          if (this.modalType == 2) {
            url = 'accommodation_category/update'
          }
          let input = vm.form
          ...
          vm.$axios.$post(url, input).then(({data}) => {
            vm.listItems = data
            vm.clearForm()
            vm.$notify('success', vm.$t('accommodation_category'), vm.$t('done'))
          })
        } else {
          vm.$notify(
            'warning',
            vm.$t('accommodation_category'),
    ...
    getAutoOrder(){
      let vm = this
      vm.$axios.$post('accommodation_category/get_auto_order', {loading: true}).then(({data}) => {
        vm.form.order = data
      })
    }
  },
