<template>
  <div>
    ...
    <i-form
      ...
      module-key="accommodation_category"
      :title="$t('accommodation_category')"
      :table-header="header"
      :table-data="items"
      :button-add="true"
      @addClick="add"
...
export default {
  layout: 'app',
  middleware: 'acl',
  moduleKey: 'accommodation_category',
  head() {
    return {
      title: this.$t('accommodation_category')
    }
  },
  components: {
  ...
  methods: {
    ...
    get(loading = true) {
      ...
      this.$axios.$post('accommodation_category/get', input).then(({data}) => {
        vm.setInput(data)
      })
    },
    /** get input */
      ...
      swal.fire({
        ...
        preConfirm: () => {
          return new Promise((resolve) => {
            ...
            input.id = item.id
            input.loading = false
            vm.$axios.$post('accommodation_category/delete', input).then(({data}) => {
              swal.close()
    ...
    statusShowSwitch(id, status, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('accommodation_category/change_status', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
