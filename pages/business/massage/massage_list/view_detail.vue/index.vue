<template>
  <div>
              ...
              <div class="mt-3">
                <b>{{ $t('payment_methods') }}</b>
                  <div v-if="$helpers.nullToVoid(bankAccountList) != ''"
                    class="mt-2 d-flex flex-wrap justify-content-between">
                    <div v-for="bank in bankAccountList" :key="bank.id"
                      ...
                      <b-row>
                        <b-col xl="4" xxs="12">
                          <b-img
                          v-if="$helpers.nullToVoid(bank.bank_image) == ''"
                          src="/default.jpg"
                          style="
                            width: 100px;
                            height: 100%;
                        ...
                        <b-col xl="8" xxs="12" class="py-3 px-2 d-flex flex-column">
    ...
    <b-card class="mt-3">
      <b-tabs>
        <b-tab :title="$t('service_list')" active>
          <b-row class="mt-3">
            <b-col class="text-left" cols="6">
              <div class="search-sm d-inline-block">
                <b-input v-model="productFilter.search" :placeholder="$t('search')" @input="getServiceList(false)"/>
              </div>
            </b-col>
            <b-col class="text-right" cols="6">
              ...
              >
                <b-dropdown-item
                  ...
                  :key='index'
                  @click='productPagination.per_page = size; getServiceList()'
                >
                  {{ size }}
                </b-dropdown-item>
              </b-dropdown>
            ...
            >
              ...
              <template #cell(image)="row">
                <i-table-image-viewer
                  ...
                  path="massage/thumbnail"
                ></i-table-image-viewer>
              </template>

              <!-- Duration -->
              <template #cell(duration)="row">
                {{ row.item.duration }} min
              </template>

              <!-- Service Type -->
              <template #cell(type)="row">
                  <span v-if="row.item.type == enums.massage_service_type.MASSAGE_AT_SHOP">
                    {{ $t('massage_at_shop') }}
                  </span>
                  <span v-else-if="row.item.type == enums.massage_service_type.MASSAGE_TO_THE_PLACE">
                    {{ $t('massage_to_the_place') }}
                  </span>
              </template>
        ...
        <b-tab :title="$t('massage_therapist')">
          <div v-if="$helpers.nullToVoid(massageTherapistList) != ''"
            class="mt-3 d-flex flex-wrap">
            <div v-for="massager in massageTherapistList" :key="massager.id">
               <div
              ...
              >
                ...
                <b-img
                  v-if="$helpers.nullToVoid(massager.profile_image) == ''"
                  src="/default.jpg"
                  style="
                    width: 170px;
                    height: 150px;
                ...
                <b-img
                  v-else
                  :src="$helpers.nullToVoid(massager.profile_image) != '' ? imagePath + 'contact/' + massager.profile_image : ''"
                  alt="thumbnail"
                  class="border-0"
                  onerror="this.src='/default.jpg'"
                  style="
                ...
                ></b-img>
                <p class="mt-2 mb-0">
                  <b>{{ $t('name') }}</b> : {{ massager.name }}
                </p>
                <p class="mt-2 mb-0">
                  <b>{{ $t('gender') }}</b> :
                  <span v-if="massager.gender == 1">
                    {{ $t('male') }}
                  </span>
                  <span v-else-if="massager.gender == 2">
                    {{ $t('female') }}
                  </span>
                </p>
                <p class="mt-2 mb-0">
                  <b>{{ $t('phone') }}</b> : {{ massager.phone }}
                </p>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getDetail(loading = true) {
      ...
      let input = {
        massage_id: vm.$route.query.id,
        loading: loading
      }

      vm.clearGenerateMap()
      vm.$axios.$post('massage_list/get_detail', input).then(({data}) => {
        vm.detailData = data
        vm.setImageGallery()
        vm.generateMap(data.latitude, data.longitude)
        vm.getServiceList()
        vm.getSelectShopDetail()
        vm.getMassageTherapistList()
      })
    },
    getServiceList(loading = true) {
      let vm = this
      ...
      vm.$axios.$post('service_list/get_service_list', input).then(({data}) => {
    ...
    getMassageTherapistList(loading = true) {
      let vm = this

      vm.massageTherapistFilter.business_id = vm.$route.query.id

      let input = {
        filter: vm.massageTherapistFilter,
        loading: loading
      }

      vm.$axios.$post('massage_therapist/get_massage_therapist_list', input).then(({data}) => {
        vm.massageTherapistList = data
      })
    },
    getSelectShopDetail(loading = true) {
      ...
      vm.$axios.$post('massage_list/get_select_massage_detail', input)
        .then(({data}) => {
    ...
    setImageGallery() {
      if (!this.$helpers.isEmpty(this.detailData)) {
        ...
        this.detailData.gallery_photo.map(obj => {
          obj.image = `${this.apiUrl}images/massage/cover/${obj.image}`
          return obj
        })
        ...
        this.detailData.gallery_photo.unshift({
          ...
          image: `${this.apiUrl}images/massage/logo/${this.detailData.image}`
        })
      }
    },
    generateMap(lat, lng) {
    ...
    suspendShowSwitch(id, suspend, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('service_list/change_status_suspend', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
