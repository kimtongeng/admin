  ...
  <div>
    ...
    <template #cell(business_type_id)="row">
      ...
      <template #cell(status)="row">
        <b-form-checkbox
          v-if="checkModuleAuthorize('event_type', 'change_status')"
...
export default {
  ...
  methods: {
    ...
    get(loading = true) {
      ...
      this.$axios.$post('event_type/get', input).then(({data}) => {
        vm.setInput(data)
      })
    },
    /** get input */
      ...
      swal.fire({
        ...
        preConfirm: () => {
          return new Promise((resolve) => {
            ...
            input.id = item.id
            input.loading = false
            vm.$axios.$post('event_type/delete', input).then(({data}) => {
              swal.close()
              vm.$notify('success', vm.$t('event_type'), vm.$t('done'))
              vm.get()
            })
          })
        },
    ...
    statusShowSwitch(id, status, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('event_type/change_status', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
