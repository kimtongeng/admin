<template>
  <div>
    ...
    <template #cell(action)="row">
        ...
        >
          <b-dropdown-item
            v-if="checkModuleAuthorize('ktv_list', 'detail')"
    ...
    <!-- Table Custom Field Area -->
      ...
      <template #cell(show)="row">
        <b-form-checkbox
          v-if="checkModuleAuthorize('ktv_list', 'change_status')"
  ...
  methods: {
    getBusinessTypeName(businessType){
      ...
      if(this.$helpers.nullToVoid(data) != ''){
        text = this.$t(this.$helpers.uppercase(data.name))
      }
    ...
    get(loading = true) {
      ...
      this.$axios.$post('ktv_list/get', input).then(({data}) => {
        vm.setInput(data)
      })
    },
    // get Input Filter
    ...
    viewDetail(item) {
      this.$router.push('/business/ktv/ktv_list/view_detail?id=' + item.id);
    },
    editItem(item){
      this.formData = item
      this.modalType = 2
    ...
    getSelectData(loading = true) {
      ...
      vm.$axios.$post('ktv_list/get_select_data', {loading: loading})
      .then(({data}) => {
    ...
    activeShowSwitch(id, active, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('ktv_list/change_active', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
    ...
    updateVerifyBusiness(id, is_verify, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('ktv_list/update_verify', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
