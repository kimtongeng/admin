  ...
  <div>
    ...
    <!-- End Action -->
      ...
      <template #cell(status)="row">
        <b-form-checkbox
          v-if="checkModuleAuthorize('vehicle_type', 'change_status')"
      ...
      <template #row-details="row">
        <b-card>
          <template v-if="$helpers.nullToVoid(row.item.vehicle_price) != ''">
            <b-row  v-for="(vehicle_price, index) in row.item.vehicle_price" :key="vehicle_price.id">
              <b-col cols="12">
                <p class="mb-1" style="font-size: 16px;">
                  <strong>{{ index + 1 }}:</strong>
                  {{ formatDistance(vehicle_price.min_distance) }} km ->
                  {{ formatDistance(vehicle_price.max_distance) }} km
                  <span class="text-right m-0">{{ vehicle_price.price }} USD</span>
                </p>
              </b-col>
            </b-row>
          </template>
...
export default {
  ...
  methods: {
    get(loading = true) {
      ...
      this.$axios.$post('vehicle_type/get', input).then(({data}) => {
        vm.setInput(data)
      })
    },
      ...
      swal.fire({
        ...
        preConfirm: () => {
          return new Promise((resolve) => {
            ...
            input.id = item.id
            input.loading = false
            vm.$axios.$post('vehicle_type/delete', input).then(({data}) => {
              swal.close()
    ...
    statusShowSwitch(id, status, index) {
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          vm.$axios
            .$post('vehicle_type/change_status', input)
            .then(({data}) => {
              swal.close()
              vm.$notify(
                'success',
